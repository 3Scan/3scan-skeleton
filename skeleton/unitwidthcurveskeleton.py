...
def getShortestPathskeleton(skeletonIm):
    ...
    startt = time.time()
    # pad the image
    skeletonIm = _getPaddedimage(skeletonIm)
...
if __name__ == '__main__':
    ...
    sample2[6, ...] = sample[0, ...]
    sample2[1:6, ...] = sample
    shortestPathSkel = getShortestPathskeleton(sample2)
    label_img1, countObjects = ndimage.measurements.label(sample2, structure=np.ones((3, 3, 3), dtype=np.uint8))
    ...
    np.save("/Users/3scan_editing/records/scratch/shortestPathSkel1.npy", shortestPathSkel)
