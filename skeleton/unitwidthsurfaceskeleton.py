...
def _setValenceOfarray(arr):
    template = np.ones((3, 3), dtype=np.uint8)
    template[1, 1] = 0
...
def _getSourcesOfShortestpaths(listNZI, valencearray):
    listIndex = [(coord, valencearray[coord]) for coord in listNZI]
    summationList = []
    if len(listIndex) != 1:
        ...
        src = listNZI[np.argmin(np.array(summationList))]
    else:
        src = listIndex[0][0]
    return src
...
def getShortestPathskeleton2D(skeletonIm):
    ...
    listNZI = list(set(map(tuple, np.transpose(np.nonzero(valencearray)))))
    if np.sum(valencearray) == 0:
        return skeletonIm[1:z + 1, 1:m + 1]
    else:
        src = _getSourcesOfShortestpaths(listNZI, valencearray)
        skeletonImNew[src] = 1
        ...
        if noOfObjects == 1 and np.sum(skeletonIm) > 50 and noOfCrowdedregions == 1:
            ...
            return np.uint8(skeletonImNew[1:z + 1, 1:m + 1])
        if np.max(skeletonLabelled) < 4:
            ...
            for i in range(0, noOfCrowdedregions):
                ...
                dilatedValenceObjectLoc = valencearray[regionLowerBoundY: regionUpperBoundY, regionLowerBoundX: regionUpperBoundX]
                dilatedLabelledObjectLoc = skeletonLabelled1[regionLowerBoundY: regionUpperBoundY, regionLowerBoundX: regionUpperBoundX]
                dilatedRegionExits = exits[regionLowerBoundY: regionUpperBoundY, regionLowerBoundX: regionUpperBoundX]
                listNZI = list(set(map(tuple, np.transpose(np.nonzero(dilatedValenceObjectLoc)))))
                src = _getSourcesOfShortestpaths(listNZI, dilatedValenceObjectLoc)
                ...
                for dest in dests:
                    ...
                    skeletonImNew[regionLowerBoundY: regionUpperBoundY, regionLowerBoundX: regionUpperBoundX] = np.logical_or(skeletonImNew[regionLowerBoundY: regionUpperBoundY, regionLowerBoundX: regionUpperBoundX], dilatedLabelledObjectLoc1)
            skeletonImNew[skeletonLabelled < 5] = 1
            skeletonImNew[skeletonLabelled == 0] = 0
            stopp = time.time()
            print("time taken to find the 2d shortest path skeleton is", (stopp - startt), "seconds")
            return np.uint8(skeletonImNew[1:z + 1, 1:m + 1])
